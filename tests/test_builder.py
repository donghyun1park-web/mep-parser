...
def test_end_to_end_build_of_the_sample_plan():
    ...
    assert ifc.count("IFCSLAB(") == st["built"]["slabs"], "슬래브 수가 안 맞는다"
    # 어떤 객체도 못 만든 레코드 = IFC 에 없는 부재. 체이닝으로 벽 수가 주는 것은
    # 정상이지만 이건 다르다 — 경고도 없이 빠진 채로 납품된다(실측: 벽 72개).
    assert st["unbuilt"] == {}, st["unbuilt"]
