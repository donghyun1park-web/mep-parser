...
def build_working_room_geometry() -> dict:
    ...
    def element(element_id, category, semantic):
        return {
            "id": element_id,
    ...
    return {
        ...
        "elements": {
            "spaces": [element("working-room-air", "space", {
                "role": "air_volume",
                "inside_dimensions_mm": [2000.0, 2000.0, 2000.0],
                "volume_m3": 8.0,
                "watertight": True,
            })],
            "equipment": [
                element("working-room-supply", "equipment", {
                    ...
                    "role": "supply",
                    "airflow_cmh": 360.0,
                    "opening_dimensions_mm": [250.0, 250.0],
                    "supply_temperature_k": 293.15,
                    "design_normal": [1.0, 0.0, 0.0],
                }),
            ...
            "heat_sources": [element("working-room-heat", "heat_source", {
                "role": "heat_source",
                ...
                "evidence": "non_authoritative_working_fixture",
                "source_type": "user_confirmed",
                "override_of_dxf": False,
            })],
        },
        "review": {
            "engine": "manual_working_validation_fixture",
            "ready": True,
            "blocking": False,
            "blocker_count": 0,
            "items": [],
            "screening_voxel_allowed": False,
        },
    }
...
def _case_metrics(label: str, record: object, root: Path, evidence: dict[str, str],
    ...
    thermal_metrics = cfd_physics.thermal_result_metrics(case, thermal)
    closure = _finite(thermal_metrics.get("energy_closure_ratio"))
    if (thermal_metrics.get("energy_closure_basis")
            != cfd_post.TRUSTED_EXHAUST_ENERGY_CLOSURE_BASIS):
    ...
    if (not isinstance(summary, dict) or not _finite_tree(summary)
            or _finite((summary.get("temperature") or {}).get("mean")) != mean_t
            or _finite((summary.get("velocity") or {}).get("mean_speed")) != mean_speed):
    ...
    for axis in "xyz":
        ...
        if (not isinstance(item, dict) or item.get("axis") != axis
                or int(item.get("sample_count") or 0) < 1 or not _finite_tree(item)):
...
def validate_sgi_screening_acceptance(
    ...
    required = set(_WORKING_ARTIFACTS) | {"field_job", "source_dxf", "reviewed_geometry"}
    ...
    if review.get("ready") is not True or review.get("blocking") is True or review.get("blocker_count") != 0:
    ...
    for item in supplies + exhausts:
        semantic = item.get("semantic") if isinstance(item.get("semantic"), dict) else {}
        normal = semantic.get("design_normal")
        if (item.get("confirmed") is not True or _finite(semantic.get("airflow_cmh")) != 444.0
            ...
            break
    if (not isinstance(geometry, dict)
            or geometry.get("scenario_authority") not in {"site_schedule", "non_authoritative_working_fixture"}
            or geometry.get("validation_fixture_only") is not True):
    ...
    if not isinstance(thermal, dict):
        blockers.append("SGI_THERMAL_INPUT_INVALID")
        return _blocked("real_dxf_screening", blockers, evidence)
    thermal_rows = thermal.get("terminals") if isinstance(thermal.get("terminals"), list) else []
    ...
    supply_total = sum(float(row.get("flow_rate_m3_s") or 0) for row in thermal_supplies)
    exhaust_total = sum(float(row.get("flow_rate_m3_s") or 0) for row in thermal_exhausts)
    ...
    if (len(thermal_supplies) != 15 or len(thermal_exhausts) != 15 or denominator <= 0
            ...
            or any(abs(float(row.get("airflow_cmh") or 0) - 444.0) > 1e-9 for row in thermal_rows)):
        blockers.append("SGI_THERMAL_TERMINAL_INPUT_INVALID")
    if (thermal.get("scenario_authority") != geometry.get("scenario_authority")
            or thermal.get("validation_fixture_only") is not True
            or _finite((thermal.get("heat") or {}).get("applied_convective_power_w")) != 15500.0):
    ...
    opening = run.get("opening_verification") if isinstance(run, dict) and isinstance(run.get("opening_verification"), dict) else {}
    if (_finite(opening.get("maximum_applied_area_error_ratio")) is None
            or float(opening["maximum_applied_area_error_ratio"]) > 0.03):
        blockers.append("SGI_OPENING_AREA_ERROR_EXCEEDED")
    if (_finite(opening.get("actual_supply_flow_error_ratio")) is None
            or float(opening["actual_supply_flow_error_ratio"]) > 0.01):
        blockers.append("SGI_SUPPLY_FLOW_ERROR_EXCEEDED")
    runtime = run.get("runtime") if isinstance(run, dict) and isinstance(run.get("runtime"), dict) else {}
    runtime_values = [_finite(runtime.get(key)) for key in (
        "runner_wall_seconds", "solver_clock_seconds", "peak_rss_bytes", "output_bytes", "available_ram_bytes"
    )]
    if any(value is None or value <= 0 for value in runtime_values):
        blockers.append("SGI_RUNTIME_EVIDENCE_INVALID")
    elif runtime_values[2] / runtime_values[4] > 0.8:
    ...
    if isinstance(job, dict):
        ...
        expected_geometry = paths["geometry"].relative_to(root).as_posix()
        ...
        if (job.get("status") not in {"complete", "analysis_complete_not_citable"}
                or job.get("stage") != "complete" or job.get("result_case") != expected_case
                or level.get("thermal_case") != expected_case
                or job_input.get("geometry_path") != expected_geometry
                or job_input.get("geometry_sha256") != _sha256_file(paths["geometry"])
                or job_input.get("source_dxf_path") != expected_dxf
...
def validate_restart_integrity(
    ...
    """Recompute restart attempts, checkpoint transition, hashes, and final binding."""
    ...
    if case is None or not {"field_job", "geometry", "thermal_input", "thermal_progress", "run", "result"}.issubset(paths):
        return _blocked("restart_integrity", blockers, evidence)
    job = _read_json(paths["field_job"])
    progress = _read_json(paths["thermal_progress"])
    run = _read_json(paths["run"])
    ...
    if not isinstance(job, dict):
        return _blocked("restart_integrity", blockers + ["RESTART_FIELD_JOB_INVALID"], evidence)
    ...
    if len(resume) != 1 or not isinstance(resume[0], dict):
        ...
        resume_row = resume[0]
    checkpoint = _finite(recovery.get("verified_checkpoint_time_s"))
    if (checkpoint is None or checkpoint <= 0
            ...
            or resume_row.get("transition") != "verified_checkpoint_to_resumed"
            or recovery.get("transition") != "verified_checkpoint_to_resumed"):
    ...
    if (recovery.get("pre_resume_geometry_sha256") != geometry_sha
            or recovery.get("post_resume_geometry_sha256") != geometry_sha
            or resume_row.get("geometry_sha256") != geometry_sha):
        blockers.append("RESTART_GEOMETRY_HASH_CHANGED")
    if (recovery.get("pre_resume_input_sha256") != input_sha
            or recovery.get("post_resume_input_sha256") != input_sha
            or resume_row.get("thermal_input_sha256") != input_sha):
        blockers.append("RESTART_INPUT_HASH_CHANGED")
    if recovery.get("duplicate_solver_count") != 0 or recovery.get("conflicting_job_relaunched") is not False:
        blockers.append("RESTART_SOLVER_CONFLICT_INVALID")
    final_time = _finite((progress or {}).get("latest_time_s"))
    flow_fraction = _finite((progress or {}).get("flow_through_fraction"))
    ...
    expected_case = case.relative_to(root).as_posix()
    level = job.get("level") if isinstance(job.get("level"), dict) else {}
    if (job.get("result_case") != expected_case or level.get("thermal_case") != expected_case
