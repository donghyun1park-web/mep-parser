...
**완료 (2026-09-16): A~G 일곱 항목 전부.** 최종 `pytest` 642 · `npm test` 32 · CLAUDE.md 338줄.
...
| 규칙 | 기호 | 면/기호 | 결과 |
|---|---|---|---|
| 환기 디퓨저 | 11곳 | 3 (19,526 · 7,960 · 7,339mm²) | 바깥 면 핸들 하나 제안 — **사람이 고른 핸들과 같다** |
| 환기 슬리브 | 2곳 | 1 (25,000mm²) | `already_single_body` — 좁힐 것 없음 |
| 난방 `EQ` | 6곳 | 1 · 4 · 5 · 7 | `shared_source` · `no_single_covering_face` — **제안 없음** |
| 난방 `SS-EQ` | 1곳 | 2 (107mm² × 2) | `shared_source` — **제안 없음** |

- **미결 "난방 분배기 핸들" 은 닫혔다 — 답은 '고를 면이 없다' 다.** `EQ` 의 면적이 305~5,180mm² 로
  분배기 본체가 아니라 밸브·헤더 **상세 기호**다. 도면에 본체 외곽이 없으므로 분배기는 계속 모델 밖이고
  난방 끊긴 끝 10개는 열림으로 남는다. 도구의 한계가 아니라 **도면 내용의 한계**다. 프로필은 바꾸지
  않았고 골든도 그대로다(도구가 읽기 전용이라 파싱 결과가 안 움직인다).
- **만들면서 파이프라인과 세 번 어긋났고 셋 다 실측으로 잡았다** — 영역 판정(걸친 원본) · 높이 묶음 ·
  닫힘/열림 키. 마지막 것이 뜻밖이다: 슬리브 사각형과 그 안의 X 표시는 원본 z 가 **9.45e-6mm** 어긋나
  `outline_groups` 에서 다른 묶음이 되고, 그래서 사각형이 X 에 사분되지 않는다. 즉 **지금의 옳은 슬리브
  결과는 도면의 부동소수 잡음에 기대고 있다.** 사실만 적고 바꾸지 않았다 — 바꾸려면 같은 도면으로 면부터
  다시 센다. (다음에 이 자리를 여는 사람이 알아야 하는 것이라 CLAUDE.md 에도 적었다.)
- `pytest` 637.
...
- 통합 모델: 간섭 CSV **24행**(손으로 짠 표와 **같은 건수**, 열 13 → 22) · 연결 CSV **110행**
  (끊긴 끝 84 · 후보 26). 손으로 짠 표에 있던 `room` 열은 그 스크립트가 사람 손으로 채운 값이라 안 만들었다.
- 버튼 이름을 '(4) 납품 검증 빌드 (FreeCAD · 느림)' 로 바꾸고, 누르면 지난 `build.json` 의
  `stage_seconds` 합과 가장 긴 단계를 먼저 찍는다. CLI 도 붙였다(`python clash_review.py … --connectivity`).
- FreeCAD 속도는 손대지 않았다 — 남은 큰 구간(IFC 122.8초 · 설비 형상 189초)이 FreeCAD 내부다(한계로 기록).
...
**계획대로 `needs_review` 를 세려다 접었다.** 실무 프로젝트는 건축 레이어 분류 확인으로 벽이 통째로 검토
대상이라 통합 모델 24건이 **전부** 걸린다(`project_architecture_classification`). 매번 24/24 를 띄우는 표시는
아무 말도 안 하는 것과 같다. 그래서 **기하의 불확실성**만 센다 — `UNCERTAIN_PAIRINGS`(`single_offset`·
`manual`)와 `GC.UNTRUSTED_WIDTH_REASONS`.

| 통합 모델 24건 | 값 |
|---|---|
| 불확실한 벽 위 | **0** (24건 전부 `paired`) |
| 그 도면의 `suspect_thin_wall` 줄 | 14 |

즉 이 도면에서 '벽이 간섭 품질의 바닥' 은 **지금은 문제가 아니다.** 10mm 칸막이는 `thin_pair` 로 안 걸리고
(그리디 페어링이 보드선을 짝지었다) 간섭 목록의 `suspect_thin_wall` 이 대신 잡는다 — 두 신호가 서로를 덮는다.
카운터는 문제가 생기면 그때 말한다.
...
- `npm test` **27 → 32**. 새로 잠근 것: 배너 문구 · 요청 본문(단건/여럿) · 일괄 버튼 · 행의
  `data-gap`·`data-confirmed` 훅 · **서버 없는 모드에서 버튼을 안 그리는 규약** · 레이어 이름이 마크업으로
  새지 않는 이스케이프. `escHtml`·`reviewRowHtml`·`batchButtonHtml` 이 `review_logic.js` 로 내려왔고
  `app.js` 는 `innerHTML` 로 붙이기만 한다. DOM 라이브러리는 넣지 않았다.
- 계획은 체크리스트를 `docs/upgrade_verification.md` 에 넣으라고 했지만 그 파일은 **2026-09-10 한 번의
  기록**이다. 되풀이할 절차를 날짜 박힌 기록에 덧붙이면 둘 다 못 읽는다 → `docs/release_checklist.md` 신설.
- A·B 에서 실제로 돈 항목(1·2·4)을 기록 표에 적었다. 3·5 는 이번 변경이 안 건드려 `—` 다 — **비워 두지
  않는다**(비면 다음 사람이 '통과' 로 읽는다).
- ★ 초안에 스캔 패턴을 그대로 적었다가 **내 스캔에 걸렸다.** 현장명을 적으면 그게 유출이고, 나머지
  토큰을 적으면 스캔이 이 문서에 매번 걸려 진짜 한 건이 묻힌다. 덮어야 할 묶음만 표로 적고 패턴은 로컬에.
...
| | 전 | 후 |
|---|---|---|
| CLAUDE.md | 1,430줄 | **338줄** |
| 결정 기록 | 없음 | `docs/decisions/` 9개 파일 (1,121줄) |
| 잠기지 않은 ★ 절 | 셈한 적 없음 | **0** — `기록만` 5건은 전부 정당한 부류 |

- **내용은 한 줄도 지우지 않았다.** 옮긴 1,121줄이 전부 어느 파일엔가 있는지 대조했다(누락 0).
- `docs/decisions/README.md` 에 절 → 테스트 표를 만들고 **인용한 테스트 이름·파일이 실재하는지 전부
  대조**했다(누락 0). 이름만 그럴듯한 표는 없느니만 못하다.
- `기록만` 이 정당한 경우를 둘로 못박았다 — 하지 말라는 기록(`pair_max` 상향·거리로 잇기)과 외부 환경의
  사실(FreeCAD 의 Arch 후처리·브라우저 번역). 둘 다 아닌데 못 적겠으면 그 절은 아직 쓸 때가 아니다.
- 큰 파일 분할은 **하지 않았다**(계획대로). 동작 변경 없는 리팩터링은 이득이 없다.
