...
2. ~~**두 번째 묶음**: #9 끊긴 끝 행 → #10 범례 숨김 → #11 3D 간섭 고리.~~ **2026-09-22 완료**(사용자 요청으로 첫 묶음 직후).
   ...
   덤: '수정 적용' 이 손대지 않은 높이 칸까지 `overrides.elevation` 으로 저장해 가정 높이가 '선언' 으로 바뀌었다 — 첫 묶음 이전부터 있던 동작.
   같은 날 고쳤다(손댄 칸만 저장 · 벽 폭 칸은 세워진 두께, [edits-and-preview.md](../decisions/edits-and-preview.md) '인스펙터의 높이 칸').
