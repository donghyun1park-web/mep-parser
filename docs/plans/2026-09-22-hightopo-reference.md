...
'계통 추적'은 직접 코드를 읽어 확인했다. 그 뒤 §2 상위 여덟을 구현하고 브라우저에서 확인했다(§6). 줄 번호는 2026-09-22 작업 트리 기준.
...
옮긴 여덟 항목을 한 묶음으로 구현했다. 새 vendor 0 · 새 geometry 키 0 · 리빌드 1회다. HighTopo 가 자랑하는 '2D/3D 공유 데이터모델 · 알람 · 경량 링크'는
...
잠금은 `tests/preview_review.test.mjs`. #1~#8 이 한 묶음(리빌드 1회) — **2026-09-22 완료**(§6).
...
| 9 | 끊긴 끝(open_ends)을 검토 행으로 | '이어졌다/안 이어졌다'가 감시 대상 | `buildReviewEntries` 에 open_ends 없음 — 평면의 점과 배너 숫자뿐이라 3D 로 갈 길이 없다. 데이터는 완비(`mep_network.py:308-312`) | 두 번째 묶음 |
| 10 | 범례 클릭으로 카테고리 숨김(벽 치우고 설비 보기) | 'Wireframe Building' · 드릴다운 | `applyEditVisuals` 의 visible 은 삭제 편집만, 범례에 click 0. `selectEid`·`fit` 이 visible 을 읽어 가드가 필요하다 | 두 번째 묶음 |
...
2. **두 번째 묶음**: #9 끊긴 끝 행 → #10 범례 숨김 → 간섭 지점의 3D 판(`THREE.Points` 1개, 클릭 광선·맞춤에 안 닿게) — 첫 묶음이 실제로 쓰이는 게 확인된 뒤.
3. **필요가 보이면**: 간섭 **지점**으로 카메라(행에 `at` 을 싣고 3m 상자로 `frameBox`) · 계통 격리 select(계통이 여럿인 도면이 생기면) ·
