...
| 선언 폭도 중심을 옮긴다 · 감싼 블록 · 동적 문은 폭 길이 직선 · 한 면 기준점은 깊이도 · 철물 원은 개구부가 아니다 | `test_a_declared_block_width_recentres_an_unnamed_window_block` · `test_a_wrapper_block_is_centred_from_the_window_block_inside_it` · `test_a_door_block_holding_several_leaf_positions_is_centred_along_its_threshold` · `test_hardware_circles_in_a_door_block_are_not_the_opening` · `test_a_window_symbol_drawn_from_one_wall_face_is_centred_in_the_wall_too` |
| 선 여러 줄로 그린 창은 한 개구부 · 마구리 선 · 문 스윙 원호 | `test_parallel_sash_lines_of_one_window_are_one_opening` · `test_two_windows_side_by_side_on_one_wall_stay_two` · `test_windows_on_both_faces_of_a_thick_wall_stay_two` · `test_a_short_line_across_the_end_of_a_window_is_its_jamb_not_an_opening` · `test_a_declared_door_swing_arc_is_not_an_opening_and_a_door_leaf_does_not_merge_with_a_window` |
| 부호는 block_map 선언으로도(`opts mark=`) · 900 은 가정 · 놓인 부호는 끊김 매칭에서 뺀다 | `test_declared_mark_joins_an_unnamed_block_to_its_schedule_row` · `test_the_900_fallback_width_of_an_unnamed_block_is_reported_assumed` · `test_rows_already_placed_by_mark_do_not_open_wall_gaps` · `test_a_mark_in_a_layer_rule_is_refused` |
