...
| 물량 요약 패널 — 저장이 곧 갱신, 재파싱 재호출 없음 · 높이 출처는 값이 아니라 키 존재로(다층 조립도 같은 신호) | `tests/test_boq.py::test_state_carries_boq_and_an_edit_updates_it_on_save` · `tests/test_preview_editing.py`(boq·level_height_declared 허용목록 2건) · `tests/preview_review.test.mjs`(4건) · `tests/test_stack.py`(다층 선언 신호 3건) |
| **pascal.md** | |
