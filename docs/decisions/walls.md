...
### ★ 길이 0 인 벽 — junction 치유가 짧은 벽의 두 끝을 한 점으로 당긴다 · 미세 선분
첫 종합평면도(층마다 A1 도곽을 나란히 놓은 xref 묶음) 기준층 한 장에서 납품 빌드의 V005(퇴화 형상)가
**길이 0 인 벽 21개**로 출력을 막았다. 단계마다 검사를 끼워 보니 페어링 뒤엔 0개, `heal_wall_junctions` 뒤에 21개였다.
- 치유는 끝점마다 이동 목표를 **원래 모양 기준으로 따로** 정하고, 가드도 '이 끝 하나만 옮기면 두께보다 짧아지나' 만 봤다.
  네 줄로 그린 310×90 작은 벽체가 **양방향으로 짝지어져** 서로 가로지르는 두 벽이 되면, 한 벽의 양 끝이 둘 다 상대 벽 중앙
  (T접합)으로 당겨져 한 점이 됐다. 두 끝이 서로를 지나쳐 **뒤집히는** 경우도 같은 구멍이다.
- 고침: 두 끝이 다 옮겨지는 벽은 **둘 다 옮긴 결과**로 다시 본다 — 두께보다 짧거나 방향이 뒤집히면 그 벽은 안 건드린다.
  실측: 그 기준층 21 → 0.
- 같이 막은 것: 폴리선의 거의 겹친 꼭짓점이 만든 **1mm 미만 선분**(`WALL_SEG_MIN_LEN_MM`)은 면선에서 뺀다 — 방향이 사실상
  난수인데 겹침 비율(짧은 쪽의 30%)만 보는 판정을 통과해 길이 0 벽이 된다. 합성 재현이고, 위 21개의 원인은 아니었다.
- 남은 것: 작은 벽체가 양방향으로 짝지어지는 것 자체(교차하는 두 벽)는 그대로다 — 모양은 한 덩어리로 겹쳐 서고, 물량은 둘로 센다.

고정: `tests/test_merge_layer.py` 의 `test_healing_never_pulls_both_ends_of_a_wall_onto_one_point` ·
`test_healing_does_not_flip_a_wall_whose_ends_cross_over` · `test_a_micro_edge_never_becomes_a_zero_length_wall`.
