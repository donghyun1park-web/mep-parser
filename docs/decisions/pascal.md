...
**2026-09-18 동결** — 코드 변경 없음. 일상 편집은 미리보기 2D 평면 탭(벽·배관·덕트·트레이 선택·
분할·결합·그리기, [edits-and-preview.md](edits-and-preview.md))으로 옮겼다. MCP 도구
(`get_pascal_snapshot`·`apply_pascal_scene`)·`pascal_host/` 의 `/pascal/*` 엔드포인트·
`run_editor.bat` 는 전부 그대로 살아 있다 — 지운 게 아니라 일상 경로가 아니라는 뜻이다. Pascal
인스펙터가 하고 평면 탭이 못 하는 것(아래 참조)이 필요할 때 여전히 연다.

### ★ Pascal 인스펙터가 하고 평면 탭이 못 하는 것 — 기록만
평면 탭(브라우저)은 기하 편집(선택·분할·결합·그리기·치수·높이)만 한다. Pascal 편집 화면의
노드 인스펙터는 그 이상을 한다:

- **계통·재질 롤 편집** — Pascal 검토 탭이 이미 보여주는 `metadata.mep` 의 계통·재질을 그
  화면에서 바로 바꾼다. 평면 탭엔 이 필드가 없다 — **§7(프로젝트 기본값·레이어 선언)이 이
  자리다**: 재질은 `layer_map.csv` 의 `material=` 이나 `source.options.defaults`(프로젝트
  기본값)로 선언하지, 뷰어에서 롤로 바꾸지 않는다(카테고리 추정을 안 한다는 규약과 같은 이유).
  계통(system, SA/RA 같은 이름)은 원래도 선언 표면이 없다 — 도면이 붙인 이름을 그대로 옮길 뿐이다.
- **단면 롤 편집** — 배관 DN·덕트 폭×높이를 프리셋 목록에서 고른다. 평면 탭은 숫자 입력만
  받는다(인스펙터 폭/지름·높이/두께 칸) — 프리셋이 없을 뿐 값은 같은 필드로 간다.
- **장면 트리** — 부재가 서로 겹쳐 클릭으로 못 고를 때 트리에서 고른다. 평면 탭은 2D 라 겹침이
  덜하지만, 겹치면 아직 대안이 없다.

일상 편집(치수·높이·분할·결합·새로 그리기)은 평면 탭으로 충분해서 동결했다 — 위 셋이 필요한
드문 경우에만 Pascal 을 연다.
