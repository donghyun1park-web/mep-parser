...
- ★ 동봉 규칙에 관행 이름 넉 줄을 더했다(`Parti`→wall `pair_min=50` · `A-FIN`·`HAT`·`Insul`→ignore).
  선매칭 우선이라 경계 없는 `Insul$` 가 아래의 `A-INSUL,wall,100` 을 가로채 지하3층 골든이 벽 662 → 648 이
  된 적이 있다 — `(^|\$)…$` 로 경계를 붙인다.
- ★ **`A-COL,wall` 은 처음엔 넣지 않았다가(대부분의 도면에서 COL 은 진짜 기둥) 2026-09-17 사용자 결정으로 넣었다.**
  GUI 대화창(`[분류 의심]` → 벽으로 다시 해석)이 넣는 바로 그 한 줄을 동봉 규칙 **맨 위**에 남기기로 했다.
  경계는 `(^|\$)A\-COL$` — 이름이 정확히 A-COL 인 것만이고 `A-COLS`·`S-COL`·`COLUMN` 은 기둥이다.
  골든 영향(커밋된 도면 넷 중): 단위세대 건축평면만 바뀐다 — **기둥 43 → 0 · 벽 40 → 91 · 검토 대기 45 → 2 · 면선커버
  99 → 100%.** 지하3층·구조 일람표·설비 두 프로젝트는 그대로다.
  **대가:** 진짜 기둥을 A-COL 로 그린 도면은 기둥이 닫힌 벽으로 가고 **경고가 없다**(증거는 '기둥 레이어가 벽처럼'
  한 방향만 잰다). 그런 도면은 로컬 layer_map 에서 이 줄을 뺀다. 증거 경로 테스트는 S-COL 로 옮겼다.
  (`test_apartment_convention_rows_do_not_shadow_the_wall_rules_below_them` ·
  `test_a_column_layer_drawn_as_walls_says_so_and_offers_the_layer_map_row`)
