...
| 8 | **미리보기 강조·선택 한 줄기** — 간섭이 있는 설비 프로젝트(`sample_mep.dxf` 는 벽이 없어 간섭 0 — 배관을 가로지르는 벽 평행선 한 쌍을 더한 사본으로) | 간섭 행 클릭 → 배관 노랑 빛 + 벽 청록 빛, 이음으로 이어진 배관 분홍 · 평면(Top) 에서 행을 눌러도 평면 그대로 · '평면 + 3D' 에서 평면 클릭 ↔ 3D 강조가 양쪽으로 따라감 · 평면에 간섭 빨간 고리 · 인스펙터 폭 칸에서 Delete·Ctrl+Z 가 부재를 안 건드림 · 폭을 고쳐 저장하면 '서버에 저장됨 · 간섭 …' 한 줄 · 저장 뒤에도 물량 머리 '층고: 열 때 선언한 값' · 끊긴 끝 행 클릭 → 그 끝으로 카메라 + 빨간 점 · 범례로 벽을 숨기면 '맞춤' 이 설비에 맞고 3D 간섭 고리는 남는다 · 숨긴 종류의 검토 행을 누르면 그 종류가 다시 보인다 · 검토 '종류' 필터가 3D 고리를 거른다 |
...
| 2026-09-22 | `1adc541` + 두 번째 묶음 | — | — | — | — | — | 8번 ✅ — 합성 도면(샘플 설비 + 벽 평행선 두 쌍, 층고 3000, 배관 `service=` 선언)에서 두 묶음 전 항목 확인, 콘솔 에러 0. 1~5 는 이번 변경이 안 건드려 생략 |
