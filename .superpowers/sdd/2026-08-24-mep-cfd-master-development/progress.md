...
- Task 7: COMPLETE (code-contract scope) — non-destructive legacy inventory/link sidecars, current Run identity validation, superseded Design computation, field job v2 identity references, v1 in-memory compatibility, pre-queue resume blocking, and Studio legacy eligibility metadata are implemented. Controller focused verification: `132 passed, 7 warnings, 9 subtests passed`; no actual field case, M2 comparison, or release evidence is claimed.
- Task 8: COMPLETE (code-contract scope) — declarative mixing/displacement HVAC templates prohibit physical defaults, require traceable direct user authority for CMH/heat/weather/comfort inputs, reject unresolved `approved_source` labels, make raw geometry preview-only after schema/recomputation checks, and require an authoritative immutable Design revision for readiness. Mapping uses stable confirmed IDs/roles and fails closed on terminal/source/balance blockers; templates select only a supported screening numerical profile. Semantic Scenario diffs use canonical exact JSON identity before presentation rounding and expand compound additions to physical field rows. Controller focused verification: `50 passed, 10 subtests passed`; extended Design/geometry/numerics/physics boundary: `145 passed, 7 skipped, 7 warnings, 27 subtests passed`; independent final review `CLEAN`. Task 9, M2, verified approved-source ingestion, actual paired Scenario runs, and release remain open.
...
- 2026-08-27 Task 7 TDD and controller verification: three project-model tests first failed because `import_legacy_case`, `link_run_identity`, and `validate_run_identity` did not exist; field v1/v2, completed-case link, Studio eligibility, resume blocking, and supersession tests then each demonstrated the missing behavior before implementation. External sidecars preserve legacy case bytes, current-reference changes return `RUN_IDENTITY_CHANGED`, and newer Design revisions return `SUPERSEDED_DESIGN_REVISION` without deletion. The specified Task 7 boundary completed with `132 passed, 7 warnings, 9 subtests passed in 28.73 s` under available Python 3.14; locked Python 3.12 full-suite authority was then supplied by the exact-commit public CI recorded below.
- 2026-08-27 Task 7 public verification: implementation/plans/progress commit `312cb2d7b3d10d420ab18b9aa58eae8aa79c21e6` was pushed to public `origin/codex/case-evidence-review-gate`. Exact-commit locked Windows CI [run `33036491175`](https://github.com/donghyun1park-web/mep-parser/actions/runs/33036491175), job `98400191067`, completed successfully in 4m39s; full pytest reported `1217 passed, 14 skipped, 7 warnings in 240.19 s`, failures/errors 0. Task 7 code-contract scope is complete; actual identity-linked field execution, Task 8, M2, and release remain open.
- 2026-08-27 Task 8 TDD and controller verification: the first template test run failed all 15 tests because the loader/apply/diff contracts did not exist. The implemented contract rejects template defaults and unattributed physics, enforces template parameter bounds, keeps raw geometry preview-only after schema/recomputation checks, validates Design from its authoritative immutable artifact, maps stable confirmed IDs/roles, records direct user authority, leaves comfort `NOT_EVALUATED`, and returns explicit blockers for missing/duplicate/unstable/unconfirmed/unknown or imbalanced terminal/source inputs. Unresolved `approved_source` labels fail closed rather than self-asserting approval. Scenario records accept profile intent without exposing solver dictionaries; the profile is an existing supported screening-only numerical profile. Semantic diff keys terminal/heat arrays by IDs, recursively expands compound additions, and compares canonical exact JSON identity before six-decimal display rounding, so integer/float and missing/null identity changes remain visible. The specified Task 8 boundary completed with `50 passed, 10 subtests passed in 6.03 s`; the extended Design/geometry/numerics/physics boundary completed with `145 passed, 7 skipped, 7 warnings, 27 subtests passed in 17.41 s` under available Python 3.14. Locked Python 3.12 full-suite authority remains pending public CI; no CFD solve, PMV/PPD evaluation, M2 comparison, verified approved-source ingestion, or release claim is made.
- 2026-08-27 Task 8 independent review: round 1 found three Important issues—caller-authored raw geometry review flags could become ready, unresolved `approved_source` labels self-asserted approval, and Python numeric equality hid JSON-identity changes such as `444` versus `444.0`. Round 2 found one Minor compound-addition presentation gap. All findings were reproduced with failing tests and fixed; final review reran `24 passed`, the combined `50 passed, 10 subtests passed`, and `git diff --check`, returning `CLEAN` with no remaining Critical/Important/Minor findings.
- 2026-08-27 Task 8 public verification: implementation/plans/progress commit `4b2794138e7ba63031e9a329c77624130ee3197f` was pushed to public `origin/codex/case-evidence-review-gate`. Exact-commit locked Windows CI [run `33041856613`](https://github.com/donghyun1park-web/mep-parser/actions/runs/33041856613), job `98417004740`, completed successfully in 4m7s; full pytest reported `1241 passed, 14 skipped, 7 warnings in 198.52 s`, failures/errors 0. Task 8 code-contract scope is complete; Task 9, M2, actual paired Scenario solves, verified approved-source ingestion, and release remain open.
