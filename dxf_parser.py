...
DEFAULT_LAYER_RULES = [
    ...
    (r"^OPEN$", "ignore", {}),
    # AutoCAD 치수 정의점(비출력). layer_map.csv 와 같은 판정 — 실측은 그쪽 주석에 있다.
    (r"^DEFPOINTS?$", "ignore", {}),
...
def detect_wall_pairs(wall_records, params):
    ...
    for k, s in enumerate(segs):
        ...
        out.append({"kind": "polyline", "closed": False,
                    ...
                    "needs_review": True, "review_reason": "single", "z_base": s.get("z_base", 0.0),
                    "layer": s.get("layer", ""),
                    "seg_length": round(seg_len, 1),
...
def repair_single_walls(wall_records, params):
    ...
    for k, s in enumerate(segs):
        ...
        out.append({"kind": "polyline", "closed": False,
                    ...
                    "needs_review": True, "review_reason": "single_offset", "z_base": s.get("z_base", 0.0),
                    "layer": s.get("layer", ""),
                    "seg_length": round(s["len"], 1),
                    "_sigs": list(s.get("sigs", [])),
                    "_span_sigs": [_span_sig([s["p1"], s["p2"]])],
